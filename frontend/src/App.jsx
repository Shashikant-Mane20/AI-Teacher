...
function LearnPage() { return <main className="flex min-h-screen items-center justify-center bg-[#030b18] px-6 text-center"><div><p className="text-xs uppercase tracking-[.25em] text-blue-400">Learning workspace</p><h1 className="mt-4 font-display text-5xl font-bold">Your lesson starts here.</h1><p className="mx-auto mt-4 max-w-md text-white/45">The interactive lesson workspace will connect to your FastAPI teaching flow here.</p><Link to="/" className="mt-8 inline-block rounded-xl bg-gradient-to-r from-[#4f89ff] to-[#7c3aed] px-6 py-3 text-sm font-semibold">Back to home</Link></div></main> }

export default function App() {
  const [authModal, setAuthModal] = useState(null)
  return <BrowserRouter><div className="min-h-screen overflow-x-hidden bg-[#030b18] text-[#e8edf8]"><Navbar onAuth={setAuthModal} /><Routes><Route path="/" element={<Home openAuth={setAuthModal} />} /><Route path="/learn" element={<LearnPage />} /></Routes>{authModal && <AuthModal mode={authModal} onClose={() => setAuthModal(null)} />}</div></BrowserRouter>
}
